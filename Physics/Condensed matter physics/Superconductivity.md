...
- From an analogy with superconductors ($\varphi \,dq\to \mu dN$), one gets the _Anderson-Josephson relation_:
...
### Flow quantisation
- Take a _holed_ geometry:
$$\oint$$
- Circulation is _quantised_

- This gives a superfluid velocity:

- There is _no screening_, and the vortex core will be very small
...
$$ \xi_{k}=\varepsilon_{k}-\mu \qquad \delta=\frac{|g|}{V}\sum_{k} \langle a_{-k}a_{k} \rangle $$
- Then with Bogoliubov decoupling, only considering _zero momentum pairs_:
